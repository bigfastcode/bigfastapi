...
class Sale(database.Base):
    __tablename__ = "sales"
    sales_id = Column(String(255), index=True, primary_key=True, default=generate_short_id(size=12))
    ...
    sale_currency = Column(String(255), nullabe=False, index=True)
...
async def fetch_sales(
    ...
    if sort_dir == "desc":
        sales = db.query(Sale, product_models.Product, customer_models.Customer, receipt_models.Receipt).filter(
    ...
    return list(map(sale_schemas.SaleBase.from_orm, sales))
...
async def fetch_sale_by_id(sale_id:str, db):
    sale = db.query(Sale, product_models.Product, customer_models.Customer, receipt_models.Receipt).filter(
