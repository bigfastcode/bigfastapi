...
def create_plan(
    ...
    if plan_search is None:
        if user.is_superuser == True:
            ...
            db.commit()
            db.refresh(plan)
            return {"message": "Plan created succesfully", "data": plan_schemas.Plan.from_orm(plan)}
    ...
    if plan_db:
        if user.is_superuser == True:
            if get_plan_by_title(title=plan.title, db=db) is None:
                ...
                plan_db = plan_db.copy(update = update_data)
                plan_db.last_updated = datetime.utcnow()
                db.commit()
                db.refresh(plan_db)
                return {"message": "Plan updated succesfully", "data": plan_schemas.Plan.from_orm(plan_db)}
            raise LookupError("plan title already exists")
