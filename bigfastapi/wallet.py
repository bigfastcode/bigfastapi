...
@app.get("/wallets/{organization_id}", response_model=schema.Wallet)
...
async def _get_wallet(organization_id: str,
    ...
    if wallet is None:
        raise fastapi.HTTPException(status_code=404, detail="Organization does not have a wallet")
...
async def debit_wallet(wallet_id: str, amount: float, db: _orm.Session):
    ...
    if wallet is None:
        raise fastapi.HTTPException(status_code=404, detail="Wallet does not exist")

    if wallet.balance - amount >= 0:
        return await _update_wallet(wallet, amount=-amount, db=db)
    else:
        raise fastapi.HTTPException(status_code=403, detail="Insufficient balance in wallet")
...
async def fund_wallet(wallet_id: str, amount: float, transaction_id: str, db: _orm.Session,
    ...
    if payment_provider is PaymentProvider.FLUTTERWAVE:
        ...
        if jsonResponse['status'] == 'pending':
            raise fastapi.HTTPException(status_code=402, detail="Transaction is pending")
            ...
            if amount == jsonResponse['data']['amount']:
                ...
                raise fastapi.HTTPException(status_code=400, detail="Transaction details do not match")
        else:
            raise fastapi.HTTPException(status_code=404, detail="Transaction not successful")
    else:
        raise fastapi.HTTPException(status_code=404, detail="Payment method not supported")
