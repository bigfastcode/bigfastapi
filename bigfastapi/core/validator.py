...
from bigfastapi.models import organisation_models, customer_models
from fastapi import HTTPException, status
from sqlalchemy.orm import Session

async def validate_organization(org_id:str, db:Session):
    organization = await organisation_models.fetchOrganization(orgId=org_id, db=db)
    if not organization:
...
async def validate_org_member(org_id:str, user_id:str, db:Session):
    is_valid_member = await organisation_models.is_organization_member(
