...
DEFAULT_MENU = {
        ...
        "more": ["reports", "invoices", "fees", "tutorials", "logs", "marketing", "sales", "suppliers", "debts", "receipts", "products", "payments"]
    }
...
def create_organization(user: users_schemas.User, db: orm.Session, organization: Schemas.OrganizationCreate):
    ...
    newOrganization = Models.Organization(id=organization_id, user_id=user.id, mission=organization.mission,
...
def get_organization_by_name(name: str, creator_id: str, db: orm.Session):
    return db.query(Models.Organization).filter(Models.Organization.name == name,
                                                 Models.Organization.user_id == creator_id).first()


async def fetch_organization_by_name(name: str, organization_id: str, db: orm.Session):
    return db.query(Models.Organization).filter(Models.Organization.name == name).filter(
...
async def organization_selector(organization_id: str, user: users_schemas.User, db: orm.Session):
    organization = (
...
async def get_organization(organization_id: str, user: users_schemas.User, db: orm.Session):
...
async def delete_organization(organization_id: str, user: users_schemas.User, db: orm.Session):
...
async def update_organization(organization_id: str, organization: Schemas.OrganizationUpdate, user: users_schemas.User,
                              db: orm.Session):
